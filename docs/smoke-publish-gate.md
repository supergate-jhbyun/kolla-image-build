...
not enable publishing by itself. The current workflow still stops the
`dry_run: false` path before any image push.
...
- `python3 scripts/plan-publish.py --profile core --release 2025.1 --distro rocky --distro-version 9 --dry-run` produces the expected `keystone` plan.
...
After explicit approval and after a later PR replaces the fail-fast guard with
real build steps, the first smoke publish should execute only the `keystone`
entries from the plan:

1. Build and push the amd64 `keystone` image.
2. Build and push the arm64 `keystone` image.
3. Inspect both per-architecture refs.
4. Create the architecture-neutral manifest tag.
